...
public class EditEmployeeActivity extends BaseActivity {
    ...
    private void initTitle() {
        mTitleView.setRightBg(R.mipmap.permissions);
    ...
    private void initIntent() {
        ...
        eetName.setText(employee.getUser_name());
        eetDept.setText(employee.getDept_name());
        eetDety.setText(employee.getDuty());
        eetRemark.setText(employee.getMemo());
        eetPhone.setText(employee.getPhone_code());
        eetZh.setText(employee.getUser_code());

        dept = new Dept(employee.getDept_name(), employee.getDept_id());

        select_sex = employee.getSex();
        select_leader = employee.getIs_leader();
        select_isnovalid = employee.getIs_novalid();
    ...
    private void initDialog() {
        dialog_sex = new AlertDialog.Builder(mContext)
                .setTitle("选择性别")
                .setSingleChoiceItems(new String[]{"男", "女"}, select_sex.equals("0")?1:0, new DialogInterface.OnClickListener() {
                    @Override
        ...
        dialog_leader = new AlertDialog.Builder(mContext)
                .setTitle("是否领导")
                .setSingleChoiceItems(new String[]{"否", "是"}, eetLeader.getText().equals("否") ? 0 : 1, new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        if (which == 0) {
                            select_leader = "0";
        ...
        dialog_isnovalid = new AlertDialog.Builder(mContext)
                .setTitle("是否注销")
                .setSingleChoiceItems(new String[]{"否", "是"}, eetIsnovalid.getText().equals("否") ? 0 : 1, new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        if (which == 0) {
                            select_isnovalid = "0";
    ...
    private void addemployeeinfo() {
        ...
        requestParams.addBodyParameter(Key.ACTION, "editemployeeinfo");
        requestParams.addBodyParameter("userid", user.getUser_id());
        requestParams.addBodyParameter("emp_id", employee.getUser_id());
